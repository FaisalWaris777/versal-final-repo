...
export default function COMPANY_COMPONENT_1(props) {
    ...
    return(
        ...
        <button onClick={()=>boxplot('Fuel_Type')}>Fuel Type</button> 
         <button onClick={()=>boxplot('Body_Type')}>Body</button></div></div>
         <div className='comp-chart'>
    <Chart options={boxplot_options} series={boxplot_series} type="boxPlot" height={'100%'} width={'100%'}/>
    </div></div>
    ...
    <button onClick={()=>power('Cylinders')}>Cylinders</button>
        <button onClick={()=>power('Displacement(in cc)')}>Displacement (cc)</button> 
         ...
         {flag==='power' ? (
           <div className='comp-chart'>
    <Chart options={scatter_options} series={scatter_series} type="scatter" height={'100%'} width={'100%'}/>
    </div>
              ) : ( 
